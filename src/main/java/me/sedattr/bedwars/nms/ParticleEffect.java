...
import me.sedattr.bedwars.nms.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
    ...
    private static boolean notWater(Location location) {
    ...
    private static boolean dataNotCorrect(ParticleEffect effect, ParticleData data) {
        return ((effect != BLOCK_CRACK && effect != BLOCK_DUST) || !(data instanceof BlockData)) && (effect != ITEM_CRACK || !(data instanceof ItemData));
    }

    private static boolean colorNotCorrect(ParticleEffect effect, ParticleColor color) {
    ...
    public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, double range) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
        ...
        if (hasProperty(ParticleProperty.REQUIRES_WATER) && notWater(center)) {
